		...
		switch(type){
			...
			case 6://json 배열 0번만 인식
				...
				if(color.length > 3)g.append('text').attr('transform', function(d) { return 'translate(' + arc.centroid(d) + ')'; }).attr('dy', '.35em').attr('fill', 'white').attr('text-anchor', 'middle').transition().delay(1000).text(function(d) { if(d.data.y >0 ) return ((d.data.y/total_cnt) * 100).toFixed(0); });
					svgG.append('text').attr('class', 'titlecenter').attr('x', parseInt(svg.style('width')) / 3).attr('y', parseInt(svg.style('height')) / 2 - 30).style('fill', 'rgb(87, 92, 98)').text('관심부위' ).style('font-size', '1.2em').attr('alignment-baseline','middle').attr('text-anchor', 'middle');

					svgG.append('text').attr('class', 'total_'+id).attr('x', parseInt(svg.style('width')) / 3).attr('y', parseInt(svg.style('height')) / 2 +20).style('fill', 'rgb(87, 92, 98)').text('%').style('font-size', '2.2em').style('font-weight','700').attr('alignment-baseline','middle').attr('text-anchor', 'middle');
//					$('.total_'+id).animateNumber({ number: 100 , numberStep: $.animateNumber.numberStepFactories.append('%')},{duration: 1000});
				...
				for(var i=0;i<title.length;i++) {
					...
					if(color.length > 3) svgG.append('text').attr('class', 'title'+i).attr('x', parseInt(svg.style('width')) - 230 ).attr('y', y_position - 60 ).style('fill', color[i]).text('● ' + data.result[0][i].item+  '\n').style('font-size', '1em').attr('alignment-baseline',' middle');
