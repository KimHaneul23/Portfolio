<?php
// include_once('./_common.php');

// if ($is_admin != 'super') die('');

include_once ('../../../common.php');
include_once('../../admin.lib.php');

include_once('../google-api-php/src/Google/autoload.php');	
include_once('../lib/lib_new.php');	


$year = $_POST['year'] == "" ? date("Y") : $_POST['year'];
$device = $_POST['device'] == "" ? "" : $_POST['device'];

// $ref_filters = "ga:deviceCategory==mobile,ga:deviceCategory==tablet";


// // // Get data from Google Analytics
$row = sql_fetch(" select * from g_analytics ");
$access_token  = $row['access_token'];
$site_id = $row['site_id'];


if($row['refresh_token'])
{
	$analytics = getAnalytics($access_token);
	$metrics = 'ga:users';
	$dimensions = 'ga:isoYear';
	
	// $cnt = 5;
	$start_date = ($year-4)."-01-01";
	$end_date = $year."-12-31";

	if($device == 'm'){
		$ref_filters = "ga:deviceCategory==mobile,ga:deviceCategory==tablet";
	}

	if($device == 'p'){
		$ref_filters = "ga:deviceCategory==desktop";
	}

	$sort = '-ga:isoYear';
		

	$data = execGa($analytics, $site_id, $start_date, $end_date, $metrics, $dimensions, $sort, $ref_filters, $cnt);
	$rows = array();
	$rows['data'] = $data['rows'];
	$rows['total'] = $data['totalsForAllResults']['ga:users'];

	print_r(json_encode($rows));

}
?>