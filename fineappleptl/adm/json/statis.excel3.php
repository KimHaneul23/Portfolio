...
if(!$_GET['sta_type']) $_GET['sta_type'] = "전체";
...
// 타이틀
->mergeCells('A1:D1')  
->setCellValue('A1',  $arr3['year']." 월간 통계 - ".$_GET['sta_type']." (".$_GET['device'].")")
->setCellValue('A2',"날짜")
->setCellValue('B2',"1년전")
->setCellValue('C2',"데이터");
...
for($i = 0; $i<count($arr3['result'][0]); $i++){
	...
	$cell_b = "B".($i+3);
	$objPHPExcel -> setActiveSheetIndex(0)
	-> setCellValue($cell_a ,$arr3['result'][0][$i]['item'])
	-> setCellValue($cell_b,$arr3['result'][0][$i]['value']);
...
for($j= 0; $j<count($arr3['result'][0]); $j++){
	$cell_c = "C".($j+3);
	$objPHPExcel -> setActiveSheetIndex(0)
	-> setCellValue($cell_c,$arr3['result'][1][$j]['value']);
...
// 셀 배경색
$objPHPExcel->getActiveSheet()->getStyle ("A1:D1")->getFill ()->getStartColor ()->setRGB ( 'D9D9D9' );
...
$filename = iconv("UTF-8", "EUC-KR", "상담신청월간".date('ymd'));
