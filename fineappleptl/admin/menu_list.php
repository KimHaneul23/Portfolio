            ...
            <tbody>
                ...
                for ($i = 0; $row = sql_fetch_array($result); $i++) {
					...
					if(strlen($row['me_code']) == 6) {
						$sub_depart = 3;
						$sub_menu_class = ' two_sub_menu_class';
						$sub_menu_info = '<span class="sound_only">'.$row['me_name'].'의 서브</span>';
						$sub_menu_ico = '<span class="sub_menu_ico"></span>';
					}
        			...
        			$first_code = '';
					if($sub_depart == 2){
						$parent_code = substr($row['me_code'], 0, 3);
					} else if($sub_depart == 3){
						$parent_code = substr($row['me_code'], 3, 2);
					} else {
						$parent_code = 0; 
					}

					if($sub_depart == 3){
						$first_code = substr($row['me_code'], 0, 3);
					} else {
						$first_code = 0; 
					}
                ?>
                    <tr class="<?php echo $bg; ?> menu_list menu_group_<?php if($sub_depart == 2){echo substr($row['me_code'], 3, 2);} else if($sub_depart == 3){echo substr($row['me_code'], 5, 1);} else {echo substr($row['me_code'], 0, 3);} ?>" data-code="<?php echo $row['me_code'];?>">
                        <td class="td_category<?php echo $sub_menu_class; ?>">
							<input type="hidden" name="code[]" value="<?php if($sub_depart == 2){echo substr($row['me_code'], 3, 2);} else if($sub_depart == 3){echo substr($row['me_code'], 5, 1);} else {echo substr($row['me_code'], 0, 3);} ?>">
							<input type="hidden" name="parentcode[]" data-parent="<?php echo $parent_code;?>" value="<?php echo $parent_code;?>">
							<input type="hidden" name="firstcode[]" data-first="<?php echo $first_code;?>" value="<?php echo $first_code;?>">
                        ...
                        <td class="td_mng">
                            <?php if(strlen($row['me_code']) == 3) { ?>
							<button type="button" class="btn_add_submenu btn_03 ">추가</button>
							<?php } ?>
							<?php if(strlen($row['me_code']) == 5) { ?>
							<button type="button" class="btn_add_two_submenu btn_03 ">추가</button>
							<?php } ?>
...
<script>
    $(function() {
        $(document).on("click", ".btn_add_submenu", function() {
            var parentcode = $(this).closest("tr").find("input[name='code[]']").val().substr(0, 3);
            var lengthChk = 0;
			$("tr").find("input[name='parentcode[]']").each(function(){
				if($(this).attr("data-parent") == parentcode){
					lengthChk++;
				}
			});
			if(lengthChk > 0){
				code = lengthChk*10 + 10;
			} else {
				code = 10;
			}

			add_submenu(parentcode, code);
        });
		
		$(document).on("click", ".btn_add_two_submenu", function() {
			var parentcode = $(this).closest("tr").find("input[name='code[]']").val().substr(0, 2);
			var firstcode = $(this).closest("tr").find("input[name='parentcode[]']").val().substr(0, 3);
			var lengthChk = 0;
			$("tr").find("input[name='parentcode[]']").each(function(){
				if($(this).attr("data-parent") == parentcode){
					if($(this).next("input[name='firstcode[]']").attr("data-first") == firstcode){
						lengthChk++;
					}
				}
			});
			...
			add_two_submenu(firstcode, parentcode, code);
		});
        ...
        $(document).on("click", ".btn_del_menu", function() {
            ...
            if($tr.find("td.two_sub_menu_class").size() > 0) {
				$tr.remove();
			} else if($tr.find("td.sub_menu_class").size() > 0) {
				var firstCode = $(this).closest("tr").attr("data-code").substr(0, 3);
				var code = $(this).closest("tr").attr("data-code").substr(3, 2);
				$(".menu_list").each(function(){
					var chkDataCode = $(this).attr("data-code").substr(0, 3);
					var chkCode = $(this).find("input[name='parentcode[]']").val();
					if(code == chkCode && firstCode == chkDataCode){
						$(this).remove();
					}
				});
				$tr.remove();
			} else {
				var code = $(this).closest("tr").attr("data-code").substr(0, 3);
				$(".menu_list").each(function(){
					var chkDataCode = $(this).attr("data-code").substr(0, 3);
					var chkCode = $(this).find("input[name='parentcode[]']").val();
					if(code == chkCode && code == chkDataCode){
						var twoCode = $(this).find("input[name='code[]']").val();
						$(this).remove();
						$(".menu_list").each(function(){
							var chkDataCode = $(this).attr("data-code").substr(0, 3);
							var twoChkCode = $(this).find("input[name='parentcode[]']").val();
							if(twoCode == twoChkCode && code == chkDataCode){
								$(this).remove();
							}
						});
					}
    ...
    function add_menu() {
        ...
        $("#menulist tr.menu_list").each(function() {
            var me_code = $(this).find("input[name='code[]']").val().substr(0, 3);
            if (max_code < me_code)
                max_code = me_code;
        });
    ...
    function add_submenu(parentcode, code) {
        var url = "./menu_form.php?parentcode="+parentcode+"&code="+code+"&new=subnew";
        window.open(url, "add_menu", "left=100,top=100,width=550,height=650,scrollbars=yes,resizable=yes");
        return false;
    }
	
	function add_two_submenu(firstcode, parentcode, code) {
		var url = "./menu_form.php?firstcode="+firstcode+"&parentcode="+parentcode+"&code="+code;
		window.open(url, "add_menu", "left=100,top=100,width=550,height=650,scrollbars=yes,resizable=yes");
		return false;
	}
