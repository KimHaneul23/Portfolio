...
if ($stx) {
    ...
    for ($i=0; $i<count($g5_search['tables']); $i++) {
        ...
        if ($row['cnt']) {
            ...
            $table_cnt[$g5_search['tables'][$i]] = $row['cnt'];
        }
    }
    ...
    //$write_pages_a = get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, $_SERVER['SCRIPT_NAME'].'?bo_table=before_after&amp;srows=6&gr_id=community&amp;'.$qstr.'&amp;page=');
    //$write_pages_b = get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, $_SERVER['SCRIPT_NAME'].'?bo_table=c_article&amp;srows=6&gr_id=community&amp;'.$qstr.'&amp;page=');
}
